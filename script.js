document.addEventListener('DOMContentLoaded', () => {
    ...
    function openGift(gift) {
        ...
        if (gift.type === 'input') {
            content = `
                <div class="modal-inner-content">
                    ...
                    <form id="wish-form" action="https://formspree.io/f/xojabokv" method="POST">
                        <textarea name="message" placeholder="Type your wish here..." required style="width: 100%; height: 100px; padding: 10px; border-radius: 5px; border: 1px solid #ccc; font-family: var(--font-sans); margin-bottom: 1rem; resize: none;"></textarea>
                        <button type="submit" id="wish-btn" style="background: var(--color-primary-red); color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; font-family: var(--font-serif); font-size: 1rem; transition: background 0.3s;">
                            Send Wish ✨
                        </button>
                        <p id="wish-status" style="margin-top: 1rem; font-size: 0.9rem; min-height: 1.2em;"></p>
                    </form>
                </div>
            `;
        }
        ...
        // Attach listener if it's the input type
        if (gift.type === 'input') {
            ...
            form.addEventListener('input', () => {
                if (status.textContent && status.style.color === 'green') {
                    status.textContent = '';
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    if (response.ok) {
                        status.textContent = "Your wish is sent to your dearest, Rhenz";
